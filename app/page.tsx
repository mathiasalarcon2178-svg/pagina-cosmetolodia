...
const SERVICES = [
  { id: 'camuflaje', name: 'Camuflaje de Estrías', price: '350.000 Gs.', duration: '1h 30m' },
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] text-[#2c2c2c] font-sans">
      {/* HEADER / NAVEGACIÓN */}
      <header className="border-b border-[#e6dfd5] bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-20 flex justify-between items-center">
          <div>
            ...
            <p className="text-[10px] text-[#8c7a6b] uppercase tracking-widest">Estética & Bienestar</p>
          </div>
          <div className="flex items-center gap-4">
            <Link 
              href="/admin" 
              className="text-xs text-[#6b5b52] hover:text-[#4a3b32] border border-[#d6ccbe] px-3 py-1.5 rounded-xl transition-all"
            >
              Acceso Admin
            </Link>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div className="space-y-6">
          <span className="bg-[#f0ebe3] text-[#4a3b32] text-xs px-3 py-1 rounded-full uppercase tracking-wider font-medium">
            Reserva tu turno online
          </span>
          <h2 className="text-3xl sm:text-5xl font-serif text-[#4a3b32] leading-tight">
            Resalta tu belleza natural con confianza.
          </h2>
          <p className="text-sm text-[#6b5b52] leading-relaxed">
            Especialistas en camuflaje de estrías, micropigmentación y tratamientos de alta calidad. Elegí el servicio que deseas, seleccioná tu fecha y agendá tu espacio en pocos pasos.
          </p>
          <div className="pt-2 flex flex-wrap gap-4 text-xs text-[#6b5b52]">
            <div className="bg-white px-4 py-2 rounded-xl border border-[#e6dfd5] shadow-sm">
              ✨ Seña: <strong className="text-[#4a3b32]">{policies.deposit} Gs.</strong>
            </div>
            <div className="bg-white px-4 py-2 rounded-xl border border-[#e6dfd5] shadow-sm">
              ⏳ Tolerancia: <strong className="text-[#4a3b32]">{policies.toleranceMinutes} min</strong>
            </div>
          </div>
        </div>

        {/* CONTENEDOR DE RESERVAS / WIDGET */}
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#e6dfd5] shadow-xl">
          {step === 1 && (
            <div className="space-y-6">
              <h3 className="text-xl font-serif text-[#4a3b32]">1. Seleccioná un Servicio</h3>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border cursor-pointer transition-all flex justify-between items-center ${
                      selectedService.id === s.id
                        ? 'border-[#4a3b32] bg-[#faf8f5] shadow-sm'
                        : 'border-[#e6dfd5] hover:border-[#d6ccbe]'
                    }`}
                  >
                    <div>
                      <h4 className="text-xs font-semibold text-[#4a3b32]">{s.name}</h4>
                      <p className="text-[11px] text-[#8c7a6b]">Duración aprox: {s.duration}</p>
                    </div>
          ...
          {step === 2 && (
            <form onSubmit={handleBookingSubmit} className="space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-serif text-[#4a3b32]">2. Fecha, Hora y Datos</h3>
                <button
                  type="button"
                  onClick={() => setStep(1)}
              ...
              >
                {loading ? 'Confirmando reserva...' : 'Confirmar y agendar turno'}
              </button>
            </form>
          )}
          ...
          {step === 3 && (
            <div className="text-center space-y-6 py-6">
              <div className="w-16 h-16 bg-[#e6dfd5] text-[#4a3b32] rounded-full flex items-center justify-center mx-auto text-2xl">
                ✓
              </div>
              <h3 className="text-2xl font-serif text-[#4a3b32]">¡Turno Reservado con Éxito!</h3>
              <p className="text-xs text-[#6b5b52] leading-relaxed">
                Gracias <strong>{clientName}</strong>. Tu turno para <strong>{selectedService.name}</strong> el día <strong>{selectedDate}</strong> a las <strong>{selectedTime} HS</strong> quedó registrado. 
              </p>
              <div className="bg-[#faf8f5] p-4 rounded-2xl border border-[#e6dfd5] text-xs text-[#6b5b52] space-y-2">
                <p>⚠️ Recuerda abonar la seña de <strong>{policies.deposit} Gs.</strong> para asegurar tu espacio definitivo.</p>
              </div>
              <button
                onClick={() => {
                  setStep(1);
